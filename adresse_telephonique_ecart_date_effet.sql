...
WITH
    ...
    last_situation_staging AS (
        ...
        FROM (
            ...
            WHERE
                code_etat = 'VA'
                AND nature_coordonnee IN ('TELP', 'MOBI')
        ) AS subquery_staging
        WHERE
            rn = 1
    ),
    -- 2. Sélection de la dernière situation connue dans TLAK_PER (avec déduplication)
    last_situation_tlak AS (
        SELECT
            CAST(refsrcper AS VARCHAR) AS num_personne,
            date_deb_effet AS date_effet_tlak
        FROM (
            SELECT
                refsrcper,
                date_deb_effet,
                ROW_NUMBER() OVER (PARTITION BY refsrcper ORDER BY date_deb_effet DESC) AS rn
            FROM tlak_per.lak_pers_adresse_telephonique
            WHERE
...
SELECT
    ...
    'lak_pers_adresse_telephonique' AS Table_Name,
