...
WITH
    ...
    last_situation_tlak AS (
        SELECT
            ...
            datdebeffem AS date_effet_tlak
        FROM (
            SELECT
                refsrcper,
                datdebeffem,
                ROW_NUMBER() OVER (PARTITION BY refsrcper ORDER BY datdebeffem DESC) AS rn
            FROM tlak_per.lak_pers_adresse_electronique
            WHERE
...
SELECT
    ...
    'lak_pers_adresse_electronique' AS Table_Name,
