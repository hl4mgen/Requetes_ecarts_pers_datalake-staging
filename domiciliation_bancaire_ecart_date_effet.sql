...
WITH
    ...
    last_situation_tlak AS (
        SELECT
            ...
            datdebvlddombq AS date_effet_tlak
        FROM (
            SELECT
                refsrcper,
                datdebvlddombq,
                ROW_NUMBER() OVER (PARTITION BY refsrcper ORDER BY datdebvlddombq DESC) AS rn
            FROM tlak_per.lak_pers_domiciliation_bancaire
            WHERE
...
SELECT
    ...
    'lak_pers_domicialisation_bancaire' AS Table_Name,
